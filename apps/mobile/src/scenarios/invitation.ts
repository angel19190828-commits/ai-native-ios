...
export function invitationToPlanDefinition({ request, plan, origin }: InvitationPlanOptions): PlanDefinition {
  ...
  const commute: PlanStepDefinition = {
    ...
    title: '计算通勤安排',
    risk: 'read',
    policy: { executor: 'server', interactionMode: 'structured', confirmation: 'once_per_plan', scopes: ['location.route'] },
    dependsOn: ['calendar'],
    input: {
  ...
  const reminders: PlanStepDefinition = {
    ...
    title: '设置准备和出发提醒',
    risk: 'write',
    policy: { executor: 'device', interactionMode: 'structured', confirmation: 'once_per_plan', scopes: ['notifications.schedule'] },
